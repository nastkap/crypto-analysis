...
def parse_performance_filename(file_path: Path):
    """Parse performance test result filename."""
    stem = file_path.stem
    # Python_Cryptography-perf-10485760-20260424-121000-summary
    pattern = re.compile(r'^(?P<node>.+?)-perf-(?P<payload>\d+)-(?P<ts>\d{8}-\d{6})-summary$')
...
def load_performance_results(results_dir: Path):
    """Load all performance test results."""
    results = {}  # {library: {payload: data}}
    
    for file in sorted(results_dir.glob('**/performance/*-summary.json')):
        try:
        ...
        metadata = parse_performance_filename(file)
        if metadata is None:
            continue
    ...
    results_json = json.dumps({
        lib: {
            str(payload): {
                ...
                'http_req_duration_p50': all_results[lib][payload]['http_req_duration_p50'],
                'http_req_duration_p95': all_results[lib][payload]['http_req_duration_p95'],
                'http_req_duration_p99': all_results[lib][payload]['http_req_duration_p99'],
          ...
          'encrypt_duration_avg': all_results[lib][payload]['encrypt_duration_avg'],
          'decrypt_duration_avg': all_results[lib][payload]['decrypt_duration_avg'],
            }
...
<body>
  <div class="wrap">
    <section class="hero">
      ...
      <p class="subtitle">Wyniki testu wydajności dla wybranej biblioteki</p>
      <div class="controls">
...
function renderChart(payloads, libraryData) {
  const labels = payloads.map(getPayloadLabel);
  const encryptData = payloads.map((payload) => libraryData[String(payload)].encrypt_duration_avg || 0);
  const decryptData = payloads.map((payload) => libraryData[String(payload)].decrypt_duration_avg || 0);

  const canvas = document.getElementById('encryptDecryptChart');
  if (!canvas) return;
...
function renderContent() {
  ...
  const tableRows = payloads.map((payload) => {
    ...
    return `
      <tr>
        ...
        <td>${data.http_req_duration_p50.toFixed(2)}</td>
        <td>${data.http_req_duration_p95.toFixed(2)}</td>
        <td>${data.http_req_duration_p99.toFixed(2)}</td>
        <td>${data.http_req_duration_avg.toFixed(2)}</td>
        <td>${data.http_reqs_rate.toFixed(2)}</td>
        <td>${data.encrypt_duration_avg.toFixed(2)}</td>
        <td>${data.decrypt_duration_avg.toFixed(2)}</td>
  ...
  section.innerHTML = `
    ...
    <div class="card chart-card">
      <h2>Sredni czas szyfrowania i deszyfrowania</h2>
      <canvas id="encryptDecryptChart"></canvas>
    </div>
  `;
  container.appendChild(section);

  renderChart(payloads, libraryData);
}
...
def main():
    parser = argparse.ArgumentParser(description='Generate performance test report with library selector.')
    parser.add_argument('--results-dir', default='perf/results', help='Directory with test results.')
    parser.add_argument('--output', default='perf/dashboard/performance-report.html', help='Output HTML file.')
