...
foreach ($target in $targets) {
    foreach ($payload in $Payloads) {
        ...
        $rawFile = Join-Path $OutDir "$($target.Name)-perf2-$payload-$timestamp-raw.json"

        Write-Host "[PERFS2] Running $($target.Name) with $VUs VU, payload=$payload bytes for $Duration"
